...
async function fetchPosts(context: QueryFunctionContext): Promise<PostsApiResponse> {
    ...
    const [_key, eventId] = context.queryKey as [string, string?]

    const pageParam = context.pageParam
    // const offset = (Number(pageNum) - 1) * Number(limit);

    const url = `/api/posts?limit=${POSTS_PER_PAGE}&offset=${pageParam}&eventId=${eventId}`;
...
export default function PostEventDetail({ eventId }: { eventId: string }) {
    ...
    return (
        < div className="space-y-6" >
            <h2 className="text-xl font-semibold text-gray-900">Diskusi Event</h2>
