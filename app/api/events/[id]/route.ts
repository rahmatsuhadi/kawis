...
export async function GET(req: Request, { params }: EventDetailParams) {
  try {
    ...
    if (error instanceof Error) {
      return NextResponse.json(
        { message: 'Failed to fetch event details', error: error.message },
        { status: 500 }
      );
    }
    return NextResponse.json(
...
export async function PATCH(req: Request, { params }: EventDetailParams) {
  try {
    ...
    if (session.user.role !== 'ADMIN') {
      return NextResponse.json(
        { message: 'Forbidden: Hanya ADMIN yang dapat mengupdate event ini.' },
        { status: 403 }
      );
    }
    ...
    if (body.startDate !== undefined && body.endDate !== undefined) {
      ...
      if (
        ...
        return NextResponse.json(
          {
            message: 'Invalid date format or end date is not after start date.',
          },
          { status: 400 }
        );
      }
      ...
      return NextResponse.json(
        {
          message:
            'Both startDate and endDate must be provided if either is updated.',
        },
        { status: 400 }
      );
    }
    ...
    if (body.latitude !== undefined && body.longitude !== undefined) {
      ...
      if (
        ...
        return NextResponse.json(
          { message: 'Invalid latitude or longitude format/range.' },
          { status: 400 }
        );
      }
      ...
      return NextResponse.json(
        {
          message:
            'Both latitude and longitude must be provided if either is updated.',
        },
        { status: 400 }
      );
    }
    ...
    if (
      body.anonymousName !== undefined &&
      body.anonymousName !== existingEvent.anonymousName
    ) {
      if (typeof body.anonymousName !== 'string') {
        return NextResponse.json(
          { message: 'Anonymous name must be a string.' },
          { status: 400 }
        );
      }
    ...
    if (
      ...
      body.organizerName !== existingEvent.organizerName
    ) {
      if (
        ...
        return NextResponse.json(
          { message: 'Organizer name must be a non-empty string.' },
          { status: 400 }
        );
      }
    ...
    if (body.status !== undefined && typeof body.status === 'string') {
      ...
      if (!['PENDING', 'APPROVED', 'REJECTED'].includes(newStatus)) {
        return NextResponse.json(
          {
            message:
              'Status event tidak valid. Harus PENDING, APPROVED, atau REJECTED.',
          },
          { status: 400 }
        );
      }
    ...
    if (
      ...
      (!body.categoryIds ||
        body.categoryIds.length === existingEvent.eventCategories.length)
    ) {
      // Cek juga apakah categoryIds yang dikirim sama persis dengan yang sudah ada (untuk mencegah update kosong)
      const currentCategoryIds = existingEvent.eventCategories
        .map((ec) => ec.categoryId)
        .sort();
      const incomingCategoryIds = (body.categoryIds || []).sort();
      if (
        JSON.stringify(currentCategoryIds) ===
        JSON.stringify(incomingCategoryIds)
      ) {
        return NextResponse.json(
          { message: 'No data provided for update or no changes detected.' },
          { status: 200 }
        );
      }
    }
    ...
    if (error instanceof Error) {
      if (
        ...
        return NextResponse.json(
          { message: 'Event name (slug) already exists.' },
          { status: 409 }
        );
      }
      return NextResponse.json(
